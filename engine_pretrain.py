...
def train_one_epoch(model: torch.nn.Module,
    ...
    return {k: meter.global_avg for k, meter in metric_logger.meters.items()}, num_steps

def train_one_epoch_revision(model: torch.nn.Module,
                    data_loader: Iterable, optimizer: torch.optim.Optimizer,
                    device: torch.device, epoch: int, loss_scaler, loss_threshold:float, num_steps = 0,
                    log_writer=None, 
                    args=None):
    ...
    for data_iter_step, (samples, _) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        with torch.cuda.amp.autocast():
            loss, _, _ = model(selected_samples, mask_ratio=args.mask_ratio)
